...
- **终端可用**：安装后 `dsh` 命令直接在你自己的终端里可用，且不受 nvm / fnm / volta 切换 Node 版本的影响。
...
- **原生终端**：如果希望使用 CLI，可通过 **菜单 → 打开终端** 启动一个已配置好 `dsh` 环境变量的终端。应用只向 PATH 添加一个目录（`%LOCALAPPDATA%\ai.deepseek.dsh.desktop\bin`），其中只有 `dsh` 一个命令，不会遮蔽你自己的 `node`、`npm`、`npx`。这个目录排在 PATH 最前面，所以终端里的 `dsh` 就是窗口里跑的那一个：它按绝对路径调用应用自带的 Node，并把自带的 `pnpm` 排在 PATH 前面，所以在终端里装插件和在面板里装的结果一致。卸载应用会把这个目录从 PATH 上移除。
- **设置**：**菜单 → 设置…** 里是开机自启、回合提醒、应用 / dsh / Node 三个版本号，以及「移除 dsh 运行时」——删掉应用自己装的那份，你自己装的 Node 和 dsh 不受影响。
...
| 环境变量 | 说明 | 默认值 |
| :--- | :--- | :--- |
| `DSH_BIN` | 指定 `dsh` 可执行文件的绝对路径（优先级最高，设置后应用不会自动更新它） | 应用自带的运行时 |
...
- **Rust**: 稳定版工具链（`stable`）
- **Node.js**：不需要预先安装。应用会在 `%LOCALAPPDATA%`（macOS/Linux 为应用数据目录）下装一份专用的 Node，与系统上已有的 Node 完全隔离，互不影响。
...
- **首次启动联网**：应用首次启动时会下载一份专用的 Node 和 dsh（合计约 220 MB）装进自己的目录，请保持网络连通。这份运行时与机器上已有的 Node 完全无关，用 nvm/fnm/volta 切换版本不会影响它。应用升级后，如果新版本换了 Node 版本，首次启动会再补装一次。
