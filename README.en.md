...
- **Smart Notifications**: A native notification when a turn finishes, or when dsh is waiting on you (a tool approval, a plan review, a question). It knows which session it is about, so clicking it returns to that one; what a single press can answer — allow/refuse, approve/revise, a two-way choice — is answered on the notification itself. Suppressed while the window is in front. Needs the Session signals plugin, which ships inside the app: one click in the plugin panel, no download. Without it the **Notifications** menu item is dimmed and says so.
...
| **Start at login** / **Notifications** | Toggles (**Notifications** needs the Session signals plugin, and is dimmed without it) |
