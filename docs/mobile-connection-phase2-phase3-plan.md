...
| （未提及） | **手机端没有任何重新配对的入口**，断了只能回电脑前用系统相机扫。而这个功能的全部意义就是人不在电脑旁 | 二.5 |
...
无论取哪条，**401 必须能被手机端识别成「需要重新配对」并给出一个不必回到电脑前的入口**，而不是一个裸的错误码。那个入口见第 5 节。
...
上面两节都以同一句话收尾：401 要能被手机端识别成「需要重新配对」。这一节说那个入口长什么样。

现状是**手机端没有任何入口**。断了就只能回到电脑前，打开卡片，用系统相机扫二维码。而 PWA 装好之后，用户最不想做的恰恰就是回到电脑前——这个功能的全部意义就是人不在电脑旁。

### 先分清两类失败

| | 服务端 | 我们的代码能不能跑 | 
| :--- | :--- | :--- |
| **A. 凭证没了，地址还在**（密钥轮换、`SESSION_TTL` 到期、点过「断开全部」、iOS PWA 独立 cookie jar） | 活着 | ✅ **已经在跑了**——`proxy.rs::unauthenticated()` 那个 401 页 |
| **B. 地址变了**（端口变、DHCP 换 IP、切通道） | 在**新**地址上活着，旧地址是死的 | ❌ 一行都跑不了 |

B 看起来是更该解决的那个，但它是卡点：**PWA 图标指向的是旧 origin**。点下去浏览器去连旧地址，没人应答，用户看到的是浏览器自己的「连接被拒」页——**我们的页面还没被加载，没有任何机会出现**。

而 A 是**更常见的那个**，并且入口已经在手上了。所以先做 A。

### A：同源重新配对（2.0b 就能做）

落点是现成的 `unauthenticated()`。那个页面现在只有一句「回到电脑上打开「手机连接」，扫一下那个二维码」，给它加一个输入框。

**用短码，不用摄像头。** 桌面卡片在二维码旁边多显示一个 **6 位短码**，网关内部把它映射到真正那 16 字节 nonce，共享同一个 [`PAIR_TTL`]（5 分钟）与一次性语义。手机在 401 页上输进去，桌面照常弹原生确认框。

- 不让用户直接输 `pair_token`：16 字节 base64 是 22 个字符，没有人会去输。
- 字母表用去掉易混字符（`I` `L` `O` `U`）的 base32，6 位约 **10.7 亿**组合。
- 安全性够：5 分钟 TTL + 一次性 + 第二章第 1 节的限流 + **桌面人工确认这道真正的闸门**。`pair_token` 本来的职责也只是「别让网络上随便谁都能按门铃」，不是身份凭证——`session.rs` 的模块文档就是这么写的。

这个方案的好处全在「它什么都不需要」上：

- **零权限、零依赖**，不碰摄像头，所以不受 secure context 限制，**明文 HTTP 的 LAN 下能用**；
- iOS 独立窗口里能用；
- **是同源导航**——从 401 页跳到同一个 origin 的 `/?pair_token=…`，**不跳出 PWA，standalone 保持，不会多出一个图标**。

**想扫码的话，回退方案是 `<input type="file" accept="image/*" capture="environment">` 加纯 JS 解码**（jsQR 一类，几十 KB）。这是普通表单控件，**不受 secure context 限制**，打开系统相机拍一张，我们自己解。比实时扫描笨，但在 LAN 明文 HTTP 下这是唯一能用的形态。

实时摄像头扫码（`getUserMedia`）要 secure context，留给 HTTPS 通道作增强，不作为主路径。

### B：离线壳（HTTPS 通道落地之后）

要让 B 成立，唯一的办法是 **Service Worker**：它注册在旧 origin 上，服务器没了也还活在浏览器里，可以拦截导航请求、从 cache 吐一个离线壳出来。这是 offline-first PWA 的标准做法，技术上完全成立。

**但 Service Worker 要求 secure context**（只有 `localhost` / `127.0.0.1` 有豁免，私有 IP 没有）。所以：

> **在 LAN 明文 HTTP 下，B 这条路是堵死的。** 没有 SW，服务端一断就没有我们的代码能跑，扫码器无处安放。

这和第六章的结论是同一件事：LAN 明文 HTTP 下 PWA 本来就是残疾的，第 4 节的固定端口只是让它别那么容易断，不是让它变成真 PWA。B 排到 HTTPS 通道之后。

### 两个躲不掉的坑

1. **跨 origin 的扫码解决不了图标。** 就算扫到了新地址，那是另一个 origin，导航过去等于离开 PWA 的 scope——iOS 上会跳出 Safari，standalone 没了，要重新「添加到主屏幕」，于是用户有两个图标。这是浏览器规则，代码绕不过去。**要在 UI 上说实话**：扫码解决的是「我能连上」，不是「我的图标还能用」。

2. **扫任意 QR 然后导航过去 = 开放重定向。** 必须校验形状（`http(s)://<host>[:<port>]/?pair_token=<32 个十六进制字符>`），并且**在跳转前把目标 host 显示给用户确认**。否则这个功能会变成一个很好用的钓鱼跳板：诱导用户在我们的应用里扫一张 QR，就把他送到任意站点。

### 后续选项：mDNS

真正治本解决「DHCP 换 IP 导致 origin 变」的是 mDNS——网关广播一个 `dsh-desktop.local`，host 就和 IP 脱钩了，origin 彻底稳定。iOS / macOS 原生支持，Android 支持参差。

比扫码治本，但要新增依赖和新增范围，列为第 4 节那三件事之后的可选项，不进 2.0b。
...
所以配对流程**必须可重入**：从主屏幕图标打开、命中未认证状态时，要能当场重新配对，而不是回一个裸的 401。

这正是第二章第 5 节那个入口要解决的 A 类失败——服务端活着、只是没凭证——而且是它最典型的一例。两边是同一件事，一起做。
...
4. **短码**：二维码旁边显示第二章第 5 节那个 6 位短码，与二维码同生共死（同一个 nonce、同一个 `PAIR_TTL`、被兑换后一起作废）。它是给**已经装了 PWA、只是掉了凭证**的手机用的——那种情况下用户不需要扫码，只需要把这六个字符输进手机上已经打开的 401 页。倒计时要显示出来，过期就重新生成。
...
| **重新配对入口** | `src-tauri/src/remote/session.rs` | 6 位短码，映射到同一个 nonce，共享 `PAIR_TTL` 与一次性语义 |
| | `src-tauri/src/remote/proxy.rs` | `unauthenticated()` 从一句话变成带输入框的重新配对页；短码兑换端点（走限流） |
...
## 里程碑 2.0b：跨重启存活与重新配对入口（PWA 的前提）

单列一个里程碑，因为第六章的 PWA 全部压在它上面，而它和任何新通道都无关——**可以现在就做，不必等 Tailscale**。前三项让配对活过重启，第四项让它万一没活过来时，用户不必回到电脑前。
...
4. 重新配对入口：短码 + 401 页上的输入框（第二章第 5 节的 A 半）。
   - **verify**：点「断开全部」→ 手机**不碰电脑、不用相机**，只把卡片上的六个字符输进它已经打开的那个页面，桌面弹确认框，点允许后回到 dsh。
   - **verify**：这一路是同源导航——iOS 上从主屏幕图标进来走完全程，**始终在独立窗口里，没有跳出 Safari，也没有多出第二个图标**。
   - **verify**：短码过期后不可兑换；兑换过一次后不可再兑换；连续输错触发限流。
