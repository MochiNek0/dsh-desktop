...
| **扫任意 QR 然后导航过去 = 开放重定向** | 若将来做页内扫码，必须校验形状（`http(s)://<host>[:<port>]/?pair_token=<22 字符 base64url>`）并**在跳转前把目标 host 显示给用户确认**，否则这是个很好用的钓鱼跳板。**注意 nonce 不是十六进制**：`session.rs::random()` 发的是 16 字节 base64url，也就是 22 个 `[A-Za-z0-9_-]`（这一行此前写的是「32 hex」，照着写出来的校验器会拒绝掉每一张真码，而失败的样子只是「扫了没反应」） |
